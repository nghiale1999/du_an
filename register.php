...
if(isset($_POST['submit'])){
    ...
    if($check == 1){
        ...
        $sql = "INSERT INTO tb_user (email, password, name, avatar) 
        VALUES ('$email', '$pass','$name','$avatar')";

        if ($result = $con->query($sql)) {
            $error = 'Đăng ký thành công mời bạn đến trang đăng nhập';
			move_uploaded_file( $file['tmp_name'],'images/'.$file['name']);
        }else{
            $error = 'Đăng ký thất bại có thể email bị trùng';
